...
class DataProcessor:
    """
    Handles the processing, cleaning, and transformation of marketing data.
    Works with Excel files exported from Power BI.
    """
    ...
    def process_excel(self, file_path: Union[str, Path]) -> pd.DataFrame:
        """
        Process an Excel file and return a cleaned DataFrame.
        
        Args:
            file_path: Path to the Excel file
            
        Returns:
            Cleaned DataFrame containing the marketing data
        """
        ...
        try:
            ...
            # Store data in database if db_manager is available
            if self.db_manager:
                upload_id = self.db_manager.store_upload(
                    filename=file_path.name,
                    df=df,
                    metadata={"imported_columns": df.columns.tolist()}
                )
                self.db_manager.store_marketing_data(upload_id, df)
                logger.info(f"Stored {len(df)} records in database with upload ID {upload_id}")
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            df: The raw dataframe from the Excel file
        ...
        # If there's no column explicitly named 'date', try to identify the main date column
        if 'date' not in df.columns and date_columns:
        ...
        df.rename(columns={col: std_col for col, std_col in column_mapping.items() 
                          if col in df.columns and std_col not in df.columns}, 
                 inplace=True)
        
        # Ensure numeric columns have proper data types
        numeric_columns = ['spend', 'roi', 'instructions', 'leads']
        ...
        # For numeric columns, fill NaNs with 0 or appropriate defaults
        for col in numeric_columns:
            if col in df.columns:
                if col == 'roi':  # Special handling for ROI
                    df[col].fillna(0, inplace=True)
                else:
                    df[col].fillna(0, inplace=True)
        
        # Validate and standardize categorical columns
        if 'channel' in df.columns:
            ...
            channel_mapping = {
                ...
                'portals': 'Aggregators',
                'property_portals': 'Aggregators',
                'rightmove': 'Aggregators',
                'zoopla': 'Aggregators',
            ...
            lowercase_mapping = {k.lower(): v for k, v in channel_mapping.items()}
            df['channel'] = df['channel'].apply(
                lambda x: lowercase_mapping.get(str(x).lower(), x) if pd.notna(x) else x
            )
        
        # Add calculated metrics if possible
        if 'spend' in df.columns and 'instructions' in df.columns and 'cost_per_instruction' not in df.columns:
            df['cost_per_instruction'] = df.apply(
                lambda row: row['spend'] / row['instructions'] if row['instructions'] > 0 else np.nan, 
                axis=1
            )
        
        if 'spend' in df.columns and 'leads' in df.columns and 'cost_per_lead' not in df.columns:
            df['cost_per_lead'] = df.apply(
                lambda row: row['spend'] / row['leads'] if row['leads'] > 0 else np.nan, 
                axis=1
            )
            
        if 'leads' in df.columns and 'instructions' in df.columns and 'conversion_rate' not in df.columns:
            df['conversion_rate'] = df.apply(
                lambda row: row['instructions'] / row['leads'] if row['leads'] > 0 else 0,
                axis=1
            )
    ...
    def get_aggregated_data(self, 
        ...
        """
        Get data aggregated by specified dimensions.
        
        Args:
            df: DataFrame to aggregate (if None, will fetch from database)
            time_period: 'daily', 'weekly', 'monthly', or 'quarterly'
            group_by: List of dimensions to group by (e.g., ['office', 'channel'])
        ...
        """
        # If no DataFrame provided, fetch from database
        if df is None and self.db_manager:
            ...
            if df.empty:
                logger.warning("No data found in database for the specified period")
                return pd.DataFrame()
        ...
        if 'date' not in df.columns:
            logger.error("Cannot aggregate: 'date' column missing from data")
            return pd.DataFrame()
